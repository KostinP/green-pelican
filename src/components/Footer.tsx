...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/texts/content.json')
      .then(res => res.json())
      .then(json => setData(json.footer as FooterData))
      .catch(() => setData(null));
  }, []);
  ...
  const defaultData: FooterData = {
    sections: [
      { 
        ...
        content: 'Формируем особую городскую среду...' 
      },
      { 
        title: 'Навигация', 
        links: [
    ...
    copyright: '© 2026 Фестиваль "Зеленый пеликан". Все права защищены.'
  };

  const footer = data || defaultData;

  return (
    <footer className="bg-[#646363] text-white py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          {footer.sections.map((section: Section, i: number) => (
            <motion.div 
              key={i} 
          ...
          >
            ...
            <div className="flex gap-4">
              {footer.socialLinks.map((social: SocialLink, i: number) => (
                <Button 
                  key={i} 
                  variant="ghost" 
                  size="icon" 
